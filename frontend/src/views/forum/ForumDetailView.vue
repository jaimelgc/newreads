...
<template>
  <div v-if="post" class="text-white">
    ...
    <p class="font-semibold text-secondary-light"><strong>{{ post.poster?.username || 'Unknown' }}</strong> - {{ new Date(post.created_at).toLocaleString() }}</p>
    <p>{{ post.content }}</p>
    ...
    <ul>
      <li v-for="comment in post.comments" :key="comment.id" style="margin-bottom: 1em;">
        <div class="border border-white p-10">
          <p class="font-semibold text-secondary-light"><strong>{{ comment.poster?.username || 'Unknown' }}</strong> said:</p>
          <p>{{ comment.content }}</p>
